...
def write_st(obj, f):
    # Encode all Unicode text to UTF-8 before writing
...
def import_st(f, obj):
    f.seek(0)
    # Read file content and decode from UTF-8 into Unicode
...
def write_native(obj, path, recursive=False):
    ...
    with open(path, "r+") as f:
        # Read file and decode as UTF-8
        lines = f.read().decode('utf-8')

        # For example, match any tags with Timestamp and replace their contents with "0"
        ...
        f.seek(0)
        # Write the modified text after encoding it to UTF-8
...
def import_dut(child, dir_path, dir_parent_obj, import_dir_fn):
    ...
    with open(os.path.join(dir_path, child), "r") as f:
        f.seek(0)
        # Read the file and decode it from UTF-8
        dut_obj.textual_declaration.replace(f.read().decode('utf-8'))
