...
def import_from_files(project):
    ...
    for device_obj in get_device_entrypoints(project):
        ...
        # Instead of remove_tracked_objects() which removes all children,
        # only remove those whose name is NOT "Configuration".
        for child in application.get_children():
            if child.get_name() == "Configuration":
                print("Keeping 'Configuration' folder.")
            else:
                print("Removing tracked object:", child.get_name())
                child.remove()
